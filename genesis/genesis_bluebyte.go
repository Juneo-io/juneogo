...
var (
	//go:embed genesis_bluebyte.json
	bluebyteGenesisConfigJSON []byte

	bluebyteMinStakeDuration time.Duration = 2 * 7 * 24 * time.Hour
	bluebyteMaxStakeDuration time.Duration = 365 * 24 * time.Hour

	// BluebyteParams are the params used for the bluebyte testnet
	BluebyteParams = Params{
		...
		StakingConfig: StakingConfig{
			...
			MinStakeDuration:  bluebyteMinStakeDuration,
			MaxStakeDuration:  bluebyteMaxStakeDuration,
			MaxValidatorWeightFactor: 10,
			RewardConfig: reward.Config{
				MinStakePeriod:         bluebyteMinStakeDuration,
				MaxStakePeriod:         bluebyteMaxStakeDuration,
