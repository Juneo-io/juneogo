...
func (b *backend) AcceptTx(ctx stdcontext.Context, tx *txs.Tx) error {
	...
	err = b.addUTXOs(ctx, constants.RelayChainID, producedUTXOSlice)
	if err != nil {
		return err
	}
...
func (b *backend) addUTXOs(ctx stdcontext.Context, destinationChainID ids.ID, utxos []*june.UTXO) error {
