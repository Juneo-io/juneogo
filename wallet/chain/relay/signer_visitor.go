...
func (s *signerVisitor) AddValidatorTx(tx *txs.AddValidatorTx) error {
	txSigners, err := s.getSigners(constants.RelayChainID, tx.Ins)
	if err != nil {
		return err
	}
...
func (s *signerVisitor) AddSupernetValidatorTx(tx *txs.AddSupernetValidatorTx) error {
	...
	supernetAuthSigners, err := s.getSupernetSigners(tx.Validator.Supernet, tx.SupernetAuth)
	if err != nil {
		return err
	}
...
func (s *signerVisitor) getSigners(sourceChainID ids.ID, ins []*june.TransferableInput) ([][]keychain.Signer, error) {
