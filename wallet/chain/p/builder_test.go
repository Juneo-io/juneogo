...
var (
	...
	testCtx = NewContext(
		...
		19*units.MicroAvax,   // CreateSupernetTxFee
		789*units.MicroAvax,  // TransformSupernetTxFee
		...
		1010*units.MilliAvax, // AddSupernetValidatorFee
		9*units.Avax,         // AddSupernetDelegatorFee
	)
)
...
func TestBaseTx(t *testing.T) {
	...
	expectedConsumed := testCtx.CreateSupernetTxFee() + outputsToMove[0].Out.Amount()
...
func TestAddSupernetValidatorTx(t *testing.T) {
	var (
		...
		backend = NewBackend(testCtx, chainUTXOs, supernets)

		// builder
		utxoAddr = utxosKey.Address()
		builder  = NewBuilder(set.Of(utxoAddr, supernetAuthAddr), backend)
	...
	expectedConsumed := testCtx.AddSupernetValidatorFee()
	consumed := ins[0].In.Amount() + ins[1].In.Amount() - outs[0].Out.Amount()
	require.Equal(expectedConsumed, consumed)
}
...
func TestCreateSupernetTx(t *testing.T) {
	...
	expectedConsumed := testCtx.CreateSupernetTxFee()
	consumed := ins[0].In.Amount() - outs[0].Out.Amount()
	require.Equal(expectedConsumed, consumed)
}
...
func TestTransformSupernetTx(t *testing.T) {
	...
	expectedConsumed := testCtx.TransformSupernetTxFee()
