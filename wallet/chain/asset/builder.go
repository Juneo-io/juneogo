...
func (b *builder) NewBaseTx(
	...
	june.SortTransferableOutputs(outputs, Parser.Codec()) // sort the outputs

	return &txs.BaseTx{BaseTx: june.BaseTx{
...
func (b *builder) NewCreateAssetTx(
	...
	toBurn := map[ids.ID]uint64{
		b.backend.JuneAssetID(): b.backend.CreateAssetTxFee(),
	}
	ops := common.NewOptions(options)
...
func (b *builder) NewImportTx(
	...
	june.SortTransferableOutputs(outputs, Parser.Codec())
	return &txs.ImportTx{
		BaseTx: txs.BaseTx{BaseTx: june.BaseTx{
			NetworkID:    b.backend.NetworkID(),
			BlockchainID: b.backend.BlockchainID(),
...
func (b *builder) spend(
	...
	june.SortTransferableOutputs(outputs, Parser.Codec()) // sort the change outputs
