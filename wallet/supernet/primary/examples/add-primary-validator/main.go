...
func main() {
	...
	// Get the Relay-chain wallet
	relayWallet := wallet.Relay()
	juneAssetID := relayWallet.JuneAssetID()

	addValidatorStartTime := time.Now()
	addValidatorTxID, err := relayWallet.IssueAddPermissionlessValidatorTx(
		&validator.SupernetValidator{Validator: validator.Validator{
			NodeID: nodeID,
