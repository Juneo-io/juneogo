...
func TestBootstrapperStartsOnlyIfEnoughStakeIsConnected(t *testing.T) {
	...
	peers.RegisterCallbackListener(ctx.SupernetID, startupTracker)
	...
	require.NoError(peers.AddStaker(commonCfg.Ctx.SupernetID, vdr0, nil, ids.Empty, startupAlpha/2))
	...
	require.NoError(peers.AddStaker(commonCfg.Ctx.SupernetID, vdr, nil, ids.Empty, startupAlpha))
