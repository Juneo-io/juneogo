...
func (cr *ChainRouter) AddChain(ctx context.Context, chain handler.Handler) {
	...
	for validatorID, peer := range cr.peers {
		...
		// connected
		if !peer.trackedSupernets.Contains(supernetID) && cr.stakingEnabled {
			continue
		}
...
func (cr *ChainRouter) Connected(nodeID ids.NodeID, nodeVersion *version.Application, supernetID ids.ID) {
	...
	if cr.stakingEnabled || supernetID == constants.PrimaryNetworkID {
		for _, chain := range cr.chainHandlers {
			...
			if supernetID == chain.Context().SupernetID || !cr.stakingEnabled {
				chain.Push(
					context.TODO(),
					handler.Message{
						InboundMessage: msg,
...
func (cr *ChainRouter) Disconnected(nodeID ids.NodeID) {
	...
	for _, chain := range cr.chainHandlers {
		if peer.trackedSupernets.Contains(chain.Context().SupernetID) || !cr.stakingEnabled {
			chain.Push(
				context.TODO(),
				handler.Message{
					InboundMessage: msg,
