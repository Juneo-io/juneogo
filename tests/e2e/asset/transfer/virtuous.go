...
var _ = e2e.DescribeXChain("[Virtuous Transfer Tx JUNE]", func() {
	ginkgo.It("can issue a virtuous transfer tx for JUNE asset",
		// use this for filtering tests by labels
		// ref. https://onsi.github.io/ginkgo/#spec-labels
		ginkgo.Label(
			"require-network-runner",
			"x",
			"virtuous-transfer-tx-june",
		),
		func() {
			...
			runFunc := func(round int) {
				...
				juneAssetID := baseWallet.Asset().JuneAssetID()
				...
				for i, w := range wallets {
					balances, err := w.Asset().Builder().GetFTBalance()
					gomega.Expect(err).Should(gomega.BeNil())

					bal := balances[juneAssetID]
					testBalances = append(testBalances, bal)

					fmt.Printf(`CURRENT BALANCE %21d JUNE (SHORT ADDRESS %q)
`,
						bal,
				...
				senderNewBal := senderOrigBal - amountToTransfer - baseWallet.Asset().BaseTxFee()
				...
				ginkgo.By("X-Chain transfer with wrong amount must fail", func() {
					...
					_, err = wallets[fromIdx].Asset().IssueBaseTx(
...
FROM [%q]
SENDER    CURRENT BALANCE     : %21d JUNE
SENDER    NEW BALANCE (AFTER) : %21d JUNE

TRANSFER AMOUNT FROM SENDER   : %21d JUNE

TO [%q]
RECEIVER  CURRENT BALANCE     : %21d JUNE
RECEIVER  NEW BALANCE (AFTER) : %21d JUNE
===
`,
					shortAddrs[fromIdx],
					senderOrigBal,
				...
				ctx, cancel := context.WithTimeout(context.Background(), e2e.DefaultConfirmTxTimeout)
				txID, err := wallets[fromIdx].Asset().IssueBaseTx(
					[]*june.TransferableOutput{{
						Asset: june.Asset{
							ID: juneAssetID,
						},
				...
				balances, err := wallets[fromIdx].Asset().Builder().GetFTBalance()
				gomega.Expect(err).Should(gomega.BeNil())
				senderCurBalX := balances[juneAssetID]
				tests.Outf("{{green}}first wallet balance:{{/}}  %d\n", senderCurBalX)

				balances, err = wallets[toIdx].Asset().Builder().GetFTBalance()
				gomega.Expect(err).Should(gomega.BeNil())
				receiverCurBalX := balances[juneAssetID]
