	...
	ginkgo.It("can issue whitelist vtx",
		...
		func() {
			...
			ginkgo.By("collect whitelist vtx metrics", func() {
				...
				if baseWallet.Relay().NetworkID() == constants.MainnetID {
			...
			ginkgo.By("issue regular, virtuous X-Chain tx, before whitelist vtx, should succeed", func() {
				balances, err := wallets[0].Asset().Builder().GetFTBalance()
				gomega.Expect(err).Should(gomega.BeNil())
				key1PrevBalX := balances[juneAssetID]
				tests.Outf("{{green}}first wallet balance:{{/}} %d\n", key1PrevBalX)

				balances, err = wallets[1].Asset().Builder().GetFTBalance()
				gomega.Expect(err).Should(gomega.BeNil())

				key2PrevBalX := balances[juneAssetID]
				...
				_, err = wallets[0].Asset().IssueBaseTx(
				...
				balances, err = wallets[0].Asset().Builder().GetFTBalance()
				gomega.Expect(err).Should(gomega.BeNil())
				key1CurBalX := balances[juneAssetID]
				tests.Outf("{{green}}first wallet balance:{{/}} %d\n", key1CurBalX)

				balances, err = wallets[1].Asset().Builder().GetFTBalance()
				gomega.Expect(err).Should(gomega.BeNil())
				key2CurBalX := balances[juneAssetID]
				tests.Outf("{{green}}second wallet balance:{{/}} %d\n", key2CurBalX)

				gomega.Expect(key1CurBalX).Should(gomega.Equal(key1PrevBalX - transferAmount - baseWallet.Asset().BaseTxFee()))
