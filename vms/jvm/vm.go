...
import (
	...
	extensions "github.com/Juneo-io/juneogo/vms/jvm/fxs"
)
...
func (vm *VM) Initialize(
	...
	if len(configBytes) > 0 {
		if err := stdjson.Unmarshal(configBytes, &jvmConfig); err != nil {
			return err
		}
		ctx.Log.Info("VM config initialized",
			zap.Reflect("config", jvmConfig),
		)
	}
	...
	vm.AddressManager = june.NewAddressManager(ctx)
	...
	vm.AtomicUTXOManager = june.NewAtomicUTXOManager(ctx.SharedMemory, vm.parser.Codec())
	...
	if jvmConfig.IndexTransactions {
		vm.ctx.Log.Info("address transaction indexing is enabled")
		vm.addressTxsIndexer, err = index.NewIndexer(vm.db, vm.ctx.Log, "", registerer, jvmConfig.IndexAllowIncomplete)
		if err != nil {
		...
		vm.addressTxsIndexer, err = index.NewNoIndexer(vm.db, jvmConfig.IndexAllowIncomplete)
		if err != nil {
...
func (vm *VM) initGenesis(genesisBytes []byte) error {
	...
	err = vm.state.SetFeesPoolValue(uint64(0))
	if err != nil {
		return err
	}

	// secure this by defaulting to juneAsset
	...
	for index, genesisTx := range genesis.Txs {
		...
		// update feeAssetID except for Asset Chain
		if index == 0 && vm.ctx.ChainID != vm.ctx.AssetChainID {
...
func (vm *VM) getUTXO(utxoID *june.UTXOID) (*june.UTXO, error) {
...
func (vm *VM) verifyTransferOfUTXO(utx txs.UnsignedTx, in *june.TransferableInput, cred verify.Verifiable, utxo *june.UTXO) error {
...
func (vm *VM) verifyTransfer(tx txs.UnsignedTx, in *june.TransferableInput, cred verify.Verifiable) error {
...
func (vm *VM) LoadUser(
	...
	*secp256k1fx.Keychain,
	...
	utxos, err := june.GetAllUTXOs(vm.state, kc.Addresses())
	if err != nil {
...
func (vm *VM) Spend(
	...
	june.SortTransferableInputsWithSigners(ins, keys)
...
func (vm *VM) SpendNFT(
	...
	for _, utxo := range utxos {
		...
		ops = append(ops, &txs.Operation{
			Asset:   utxo.Asset,
			UTXOIDs: []*june.UTXOID{&utxo.UTXOID},
...
func (vm *VM) selectChangeAddr(defaultAddr ids.ShortID, changeAddr string) (ids.ShortID, error) {
	...
	addr, err := june.ParseServiceAddress(vm, changeAddr)
	if err != nil {
