...
func (tx *UniqueTx) Accept(context.Context) error {
	...
	inputUTXOs := make([]*june.UTXO, 0, len(inputUTXOIDs))
	...
	feesPoolValue, err := tx.vm.state.GetFeesPoolValue()
	if err != nil {
		return fmt.Errorf("error retrieving fees pool value for tx: %w", err)
	}
	newFeesPoolValue, err := math.Add64(feesPoolValue, tx.Unsigned.ConsumedValue(tx.vm.feeAssetID))
	if err != nil {
		return fmt.Errorf("error setting new fees pool value for tx: %w", err)
	}
	tx.vm.state.SetFeesPoolValue(newFeesPoolValue)
...
func (tx *UniqueTx) InputUTXOs() []*june.UTXOID {
	tx.refresh()
...
func (tx *UniqueTx) UTXOs() []*june.UTXO {
	tx.refresh()
