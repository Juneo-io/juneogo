...
func TestInitialStateVerifyInvalidOutput(t *testing.T) {
	c := linearcodec.NewDefault()
	if err := c.RegisterType(&june.TestVerifiable{}); err != nil {
		t.Fatal(err)
	}
	m := codec.NewDefaultManager()
	if err := m.RegisterCodec(CodecVersion, c); err != nil {
	...
	is := InitialState{
		FxIndex: 0,
		Outs:    []verify.State{&june.TestVerifiable{Err: errors.New("")}},
	}
	if err := is.Verify(m, numFxs); err == nil {
...
func TestInitialStateVerifyUnsortedOutputs(t *testing.T) {
	c := linearcodec.NewDefault()
	if err := c.RegisterType(&june.TestTransferable{}); err != nil {
		t.Fatal(err)
	}
	m := codec.NewDefaultManager()
	if err := m.RegisterCodec(CodecVersion, c); err != nil {
	...
	is := InitialState{
		FxIndex: 0,
		Outs: []verify.State{
			&june.TestTransferable{Val: 1},
			&june.TestTransferable{Val: 0},
		},
	}
	if err := is.Verify(m, numFxs); err == nil {
