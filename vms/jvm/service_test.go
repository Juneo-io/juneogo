...
func setup(t *testing.T, isJUNEAsset bool) ([]byte, *VM, *Service, *atomic.Memory, *txs.Tx) {
...
func setupWithIssuer(t *testing.T, isJUNEAsset bool) ([]byte, *VM, *Service, chan common.Message) {
	var genesisBytes []byte
	var vm *VM
...
// 3) The service that wraps the VM
// 4) atomic memory to use in tests
func setupWithKeys(t *testing.T, isJUNEAsset bool) ([]byte, *VM, *Service, *atomic.Memory, *txs.Tx) {
	genesisBytes, vm, s, m, tx := setup(t, isJUNEAsset)
...
func TestServiceGetAllBalances(t *testing.T) {
	...
	otherAssetUTXO := &june.UTXO{
		UTXOID: june.UTXOID{
			TxID:        ids.GenerateTestID(),
			OutputIndex: 0,
		},
		Asset: june.Asset{ID: otherAssetID},
		Out: &secp256k1fx.TransferOutput{
			Amt: 1337,
			OutputOwners: secp256k1fx.OutputOwners{
				Threshold: 2,
...
func TestServiceGetTxJSON_BaseTx(t *testing.T) {
	...
	newTx := newJuneBaseTxWithOutputs(t, genesisBytes, vm)
...
func TestServiceGetTxJSON_ExportTx(t *testing.T) {
	...
	newTx := newJuneExportTxWithOutputs(t, genesisBytes, vm)
...
func TestServiceGetTxJSON_CreateAssetTx(t *testing.T) {
	...
	createAssetTx := newJuneCreateAssetTxWithOutputs(t, vm)
...
func newJuneBaseTxWithOutputs(t *testing.T, genesisBytes []byte, vm *VM) *txs.Tx {
	juneTx := GetJUNETxFromGenesisTest(genesisBytes, t)
	key := keys[0]
	tx := buildBaseTx(juneTx, vm, key)
...
func newJuneExportTxWithOutputs(t *testing.T, genesisBytes []byte, vm *VM) *txs.Tx {
	juneTx := GetJUNETxFromGenesisTest(genesisBytes, t)
	key := keys[0]
	tx := buildExportTx(juneTx, vm, key)
	if err := tx.SignSECP256K1Fx(vm.parser.Codec(), [][]*crypto.PrivateKeySECP256K1R{{key}}); err != nil {
		t.Fatal(err)
	}
	return tx
...
func newJuneCreateAssetTxWithOutputs(t *testing.T, vm *VM) *txs.Tx {
	key := keys[0]
...
func buildBaseTx(juneTx *txs.Tx, vm *VM, key *crypto.PrivateKeySECP256K1R) *txs.Tx {
...
func buildExportTx(juneTx *txs.Tx, vm *VM, key *crypto.PrivateKeySECP256K1R) *txs.Tx {
	return &txs.Tx{Unsigned: &txs.ExportTx{
		...
		DestinationChain: constants.RelayChainID,
		ExportedOuts: []*june.TransferableOutput{{
			Asset: june.Asset{ID: juneTx.ID()},
			Out: &secp256k1fx.TransferOutput{
				Amt: startBalance - vm.TxFee,
				OutputOwners: secp256k1fx.OutputOwners{
...
func buildNFTxMintOp(createAssetTx *txs.Tx, key *crypto.PrivateKeySECP256K1R, outputIndex, groupID uint32) *txs.Operation {
	return &txs.Operation{
		Asset: june.Asset{ID: createAssetTx.ID()},
		UTXOIDs: []*june.UTXOID{{
...
func TestServiceGetUTXOs(t *testing.T) {
	...
	sm := m.NewSharedMemory(constants.RelayChainID)
	...
	pAddr, err := vm.FormatAddress(constants.RelayChainID, rawAddr)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCreateFixedCapAsset(t *testing.T) {
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, vm, s, _, _ := setupWithKeys(t, tc.juneAsset)
			defer func() {
				if err := vm.Shutdown(context.Background()); err != nil {
					t.Fatal(err)
				}
...
func TestImportJVMKeyNoDuplicates(t *testing.T) {
	_, vm, s, _, _ := setup(t, true)
	ctx := vm.ctx
	defer func() {
		if err := vm.Shutdown(context.Background()); err != nil {
...
func TestSendMultiple(t *testing.T) {
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, vm, s, _, genesisTx := setupWithKeys(t, tc.juneAsset)
			defer func() {
				if err := vm.Shutdown(context.Background()); err != nil {
					t.Fatal(err)
				}
...
func TestImport(t *testing.T) {
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			_, vm, s, m, genesisTx := setupWithKeys(t, tc.juneAsset)
			defer func() {
				if err := vm.Shutdown(context.Background()); err != nil {
					t.Fatal(err)
				}
			...
			utxo := &june.UTXO{
				UTXOID: june.UTXOID{TxID: ids.Empty},
				Asset:  june.Asset{ID: assetID},
				Out: &secp256k1fx.TransferOutput{
					Amt: 7,
					OutputOwners: secp256k1fx.OutputOwners{
			...
			peerSharedMemory := m.NewSharedMemory(constants.RelayChainID)
			utxoID := utxo.InputID()
			...
			if err := s.Import(nil, args, reply); err != nil {
				t.Fatalf("Failed to import JUNE due to %s", err)
			}
		})
	}
}
