...
func (w *WalletService) update(utxos []*june.UTXO) ([]*june.UTXO, error) {
	utxoMap := make(map[ids.ID]*june.UTXO, len(utxos))
...
func (w *WalletService) IssueTx(_ *http.Request, args *api.FormattedTx, reply *api.JSONTxID) error {
	w.vm.ctx.Log.Debug("JVM Wallet: IssueTx called",
...
func (w *WalletService) SendMultiple(_ *http.Request, args *SendMultipleArgs, reply *api.JSONTxIDChangeAddr) error {
	w.vm.ctx.Log.Debug("JVM Wallet: SendMultiple",
	...
	fromAddrs, err := june.ParseServiceAddresses(w.vm, args.From)
	if err != nil {
	...
	for _, output := range args.Outputs {
		...
		to, err := june.ParseServiceAddress(w.vm, output.To)
		if err != nil {
	...
	june.SortTransferableOutputs(outs, codec)
