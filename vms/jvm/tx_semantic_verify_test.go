...
func TestBaseTxSemanticVerifyUnauthorizedFx(t *testing.T) {
	...
	fx.InitializeF = func(vmIntf interface{}) error {
		vm := vmIntf.(secp256k1fx.VM)
		return vm.CodecRegistry().RegisterType(&june.TestTransferable{})
	}
		...
		BaseTx: txs.BaseTx{BaseTx: june.BaseTx{
			...
			Ins: []*june.TransferableInput{
				...
				},
				{
					...
					Asset: june.Asset{ID: assetID}, // This asset doesn't exist
					In: &secp256k1fx.TransferInput{
						Amt:   startBalance,
						Input: secp256k1fx.Input{SigIndices: []uint32{0}},
					},
...
func TestExportTxSemanticVerifyInvalidFx(t *testing.T) {
	...
	utx.Tx.Creds[0].Verifiable = &june.TestVerifiable{}
	err = rawTx.Unsigned.Visit(&txSemanticVerify{
		tx: utx.Tx,
		vm: vm,
	})
...
func TestExportTxSemanticVerifyTransferCustomAsset(t *testing.T) {
	...
	genesisJuneTx := GetJUNETxFromGenesisTest(genesisBytes, t)
	juneID := genesisJuneTx.ID()
