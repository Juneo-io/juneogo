...
func TestRemovePermissionedValidatorDuringPendingToCurrentTransitionNotTracked(t *testing.T) {
	...
	require.NoError(vm.Builder.AddUnverifiedTx(addSupernetValidatorTx))
	...
	require.NoError(vm.Builder.AddUnverifiedTx(removeSupernetValidatorTx))
...
func TestRemovePermissionedValidatorDuringPendingToCurrentTransitionTracked(t *testing.T) {
	...
	require.NoError(vm.state.ApplyCurrentValidators(createSupernetTx.ID(), vm.Validators))
...
func TestSupernetValidatorBLSKeyDiffAfterExpiry(t *testing.T) {
	...
	supernetTx, err := vm.txBuilder.NewAddSupernetValidatorTx(
		1,                              // Weight
		uint64(supernetStartTime.Unix()), // Start time
		uint64(supernetEndTime.Unix()),   // end time
		nodeID,                         // Node ID
		supernetID,
		[]*secp256k1.PrivateKey{keys[0], keys[1]},
		addr,
	...
	require.NoError(vm.Builder.AddUnverifiedTx(supernetTx))
	require.NoError(buildAndAcceptStandardBlock(vm))
