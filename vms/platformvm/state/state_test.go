	...
	tests := map[string]struct {
		...
		"add current validator": {
			...
			checkDiffs: func(r *require.Assertions, s *state, staker *Staker, height uint64) {
				weightDiffBytes, err := s.flatValidatorWeightDiffsDB.Get(marshalDiffKey(staker.SupernetID, height, staker.NodeID))
				r.NoError(err)
				...
				blsDiffBytes, err := s.flatValidatorPublicKeyDiffsDB.Get(marshalDiffKey(staker.SupernetID, height, staker.NodeID))
		...
		"add pending validator": {
			...
			checkDiffs: func(r *require.Assertions, s *state, staker *Staker, height uint64) {
				...
				_, err := s.flatValidatorWeightDiffsDB.Get(marshalDiffKey(staker.SupernetID, height, staker.NodeID))
				r.ErrorIs(err, database.ErrNotFound)

				_, err = s.flatValidatorPublicKeyDiffsDB.Get(marshalDiffKey(staker.SupernetID, height, staker.NodeID))
				r.ErrorIs(err, database.ErrNotFound)
			},
		},
