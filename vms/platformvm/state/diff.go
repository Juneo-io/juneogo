...
type diff struct {
	...
	// Supernet ID --> rewards pool supply of native asset of the supernet
...
func (d *diff) GetRewardsPoolSupply(supernetID ids.ID) (uint64, error) {
	rewardsPoolSupply, ok := d.rewardsPoolSupply[supernetID]
	if ok {
		return rewardsPoolSupply, nil
	}

	// If the supernet rewards pool supply wasn't modified in this diff, ask the parent state.
	parentState, ok := d.stateVersions.GetState(d.parentID)
	if !ok {
		return 0, fmt.Errorf("%w: %s", ErrMissingParentState, d.parentID)
	}
...
func (d *diff) Apply(baseState State) error {
	...
	for supernetID, rewardsPoolSupply := range d.rewardsPoolSupply {
		baseState.SetRewardsPoolSupply(supernetID, rewardsPoolSupply)
	}
