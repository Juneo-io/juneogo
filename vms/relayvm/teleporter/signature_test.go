	...
	tests := []struct {
	...
	}{
		{
			...
			stateF: func(ctrl *gomock.Controller) validators.State {
				...
				state.EXPECT().GetSupernetID(gomock.Any(), sourceChainID).Return(supernetID, errMock)
				return state
			},
			quorumNum: 1,
			quorumDen: 2,
		...
		},
		{
			...
			stateF: func(ctrl *gomock.Controller) validators.State {
				...
				state.EXPECT().GetValidatorSet(gomock.Any(), pChainHeight, supernetID).Return(nil, errMock)
				return state
			},
			quorumNum: 1,
			quorumDen: 2,
