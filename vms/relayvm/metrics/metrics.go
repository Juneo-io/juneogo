...
func New(
	...
	m := &metrics{
		...
		localStake: prometheus.NewGauge(prometheus.GaugeOpts{
			Namespace: namespace,
			Name:      "local_staked",
			Help:      "Amount (in nJune) of JUNE staked on this node",
		}),
		totalStake: prometheus.NewGauge(prometheus.GaugeOpts{
			Namespace: namespace,
			Name:      "total_staked",
			Help:      "Amount (in nJune) of JUNE staked on the Primary Network",
		}),
	...
	// init supernet tracker metrics with whitelisted supernets
	for supernetID := range whitelistedSupernets {
		// initialize to 0
