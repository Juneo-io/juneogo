...
func Consume(utxoDB state.UTXODeleter, ins []*june.TransferableInput) {
...
type Spender interface {
	...
	// - [fee] is the amount of JUNE that should be burned
	...
	Spend(
		...
		[]*june.TransferableInput, // inputs
		[]*june.TransferableOutput, // returnedOutputs
		[]*june.TransferableOutput, // stakedOutputs
...
func (h *handler) Spend(
	...
	utxos, err := june.GetAllUTXOs(h.utxosReader, addrs) // The UTXOs controlled by [keys]
	if err != nil {
	...
	ins := []*june.TransferableInput{}
	returnedOuts := []*june.TransferableOutput{}
	stakedOuts := []*june.TransferableOutput{}
	signers := [][]*crypto.PrivateKeySECP256K1R{}

	// Amount of JUNE that has been staked
	amountStaked := uint64(0)

	// Consume locked UTXOs
	for _, utxo := range utxos {
		// If we have consumed more JUNE than we are trying to stake, then we
		// have no need to consume more locked JUNE
		if amountStaked >= amount {
			break
		}

		if assetID := utxo.AssetID(); assetID != h.ctx.JuneAssetID {
			continue // We only care about staking JUNE, so ignore other assets
		}
		...
		in, ok := inIntf.(june.TransferableIn)
		if !ok { // should never happen
			h.ctx.Log.Warn("wrong input type",
				zap.String("expectedType", "june.TransferableIn"),
		...
		ins = append(ins, &june.TransferableInput{
			UTXOID: utxo.UTXOID,
		...
		stakedOuts = append(stakedOuts, &june.TransferableOutput{
			Asset: june.Asset{ID: h.ctx.JuneAssetID},
		...
		if remainingValue > 0 {
			...
			returnedOuts = append(returnedOuts, &june.TransferableOutput{
				Asset: june.Asset{ID: h.ctx.JuneAssetID},
				Out: &stakeable.LockOut{
					Locktime: out.Locktime,
					TransferableOut: &secp256k1fx.TransferOutput{
	...
	// Amount of JUNE that has been burned
	amountBurned := uint64(0)

	for _, utxo := range utxos {
		// If we have consumed more JUNE than we are trying to stake,
		// and we have burned more JUNE than we need to,
		// then we have no need to consume more JUNE
		if amountBurned >= fee && amountStaked >= amount {
			break
		}

		if assetID := utxo.AssetID(); assetID != h.ctx.JuneAssetID {
			continue // We only care about burning JUNE, so ignore other assets
		}
	...
	june.SortTransferableInputsWithSigners(ins, signers)  // sort inputs and keys
	june.SortTransferableOutputs(returnedOuts, txs.Codec) // sort outputs
	june.SortTransferableOutputs(stakedOuts, txs.Codec)   // sort outputs
...
func (h *handler) Authorize(
	...
	supernetTx, _, err := state.GetTx(supernetID)
	if err != nil {
		return nil, nil, fmt.Errorf(
			"failed to fetch supernet %s: %w",
			supernetID,
			err,
		)
	}
	...
	if !ok {
		return nil, nil, fmt.Errorf("expected tx type *txs.CreateSupernetTx but got %T", supernetTx.Unsigned)
	}

	// Make sure the owners of the supernet match the provided keys
	owner, ok := supernet.Owner.(*secp256k1fx.OutputOwners)
	if !ok {
		return nil, nil, fmt.Errorf("expected *secp256k1fx.OutputOwners but got %T", supernet.Owner)
	}
