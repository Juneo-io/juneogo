...
func TestDiffStakersValidator(t *testing.T) {
	...
	_, ok = v.GetValidator(delegator.SupernetID, ids.GenerateTestNodeID())
	require.False(ok)

	_, ok = v.GetValidator(delegator.SupernetID, delegator.NodeID)
	require.False(ok)
	...
	returnedStaker, ok := v.GetValidator(staker.SupernetID, staker.NodeID)
	require.True(ok)
	require.Equal(staker, returnedStaker)
