...
type Chain interface {
	...
	GetRewardUTXOs(txID ids.ID) ([]*june.UTXO, error)
	AddRewardUTXO(txID ids.ID, utxo *june.UTXO)
	...
	AddTx(tx *txs.Tx, status status.Status, feesAssetID ids.ID)
}
...
type state struct {
	...
	validatorWeightDiffsCache cache.Cacher // cache of heightWithSupernet -> map[ids.NodeID]*ValidatorWeightDiff
	...
	addedRewardUTXOs map[ids.ID][]*june.UTXO // map of txID -> []*UTXO
	rewardUTXOsCache cache.Cacher            // cache of txID -> []*UTXO
	rewardUTXODB     database.Database

	modifiedUTXOs map[ids.ID]*june.UTXO // map of modified UTXOID -> *UTXO if the UTXO is nil, it has been removed
	utxoDB        database.Database
	utxoState     june.UTXOState
	...
	transformedSupernetCache cache.Cacher       // cache of supernetID -> transformSupernetTx if the entry is nil, it is not in the database
	transformedSupernetDB    database.Database

	modifiedSupplies map[ids.ID]uint64 // map of supernetID -> current supply
	supplyCache      cache.Cacher      // cache of supernetID -> current supply if the entry is nil, it is not in the database
	supplyDB         database.Database

	modifiedRewardsSupplies map[ids.ID]uint64 // map of supernetID -> rewards pool supply
	rewardsSupplyCache      cache.Cacher      // cache of supernetID -> rewards pool supply if the entry is nil, it is not in the database
	rewardsSupplyDB         database.Database

	addedChains  map[ids.ID][]*txs.Tx // maps supernetID -> the newly added chains to the supernet
	chainCache   cache.Cacher         // cache of supernetID -> the chains after all local modifications []*txs.Tx
	chainDBCache cache.Cacher         // cache of supernetID -> linkedDB
...
func new(
	...
	utxoState, err := june.NewMeteredUTXOState(utxoDB, txs.GenesisCodec, metricsReg)
	if err != nil {
		return nil, err
	}
	...
	transformedSupernetCache, err := metercacher.New(
	...
	rewardsSupplyCache, err := metercacher.New(
	...
	return &state{
		...
		addedRewardUTXOs: make(map[ids.ID][]*june.UTXO),
		rewardUTXODB:     rewardUTXODB,
		rewardUTXOsCache: rewardUTXOsCache,

		modifiedUTXOs: make(map[ids.ID]*june.UTXO),
		utxoDB:        utxoDB,
		utxoState:     utxoState,
...
func (s *state) GetSupernetTransformation(supernetID ids.ID) (*txs.Tx, error) {
	...
	if txIntf, cached := s.transformedSupernetCache.Get(supernetID); cached {
		if txIntf == nil {
...
func (s *state) GetChains(supernetID ids.ID) ([]*txs.Tx, error) {
	if chainsIntf, cached := s.chainCache.Get(supernetID); cached {
...
func (s *state) getChainDB(supernetID ids.ID) linkeddb.LinkedDB {
	if chainDBIntf, cached := s.chainDBCache.Get(supernetID); cached {
...
func (s *state) AddTx(tx *txs.Tx, status status.Status, feesAssetID ids.ID) {
...
func (s *state) GetRewardUTXOs(txID ids.ID) ([]*june.UTXO, error) {
	if utxos, exists := s.addedRewardUTXOs[txID]; exists {
		return utxos, nil
	}
	if utxos, exists := s.rewardUTXOsCache.Get(txID); exists {
		return utxos.([]*june.UTXO), nil
	}
...
func (s *state) AddRewardUTXO(txID ids.ID, utxo *june.UTXO) {
	s.addedRewardUTXOs[txID] = append(s.addedRewardUTXOs[txID], utxo)
}

func (s *state) GetUTXO(utxoID ids.ID) (*june.UTXO, error) {
...
func (s *state) AddUTXO(utxo *june.UTXO) {
...
func (s *state) GetCurrentSupply(supernetID ids.ID) (uint64, error) {
	...
	supplyIntf, ok := s.supplyCache.Get(supernetID)
	if ok {
		if supplyIntf == nil {
...
func (s *state) GetRewardsPoolSupply(supernetID ids.ID) (uint64, error) {
	...
	rewardsSupplyIntf, ok := s.rewardsSupplyCache.Get(supernetID)
	if ok {
		if rewardsSupplyIntf == nil {
			return 0, database.ErrNotFound
		}
		return rewardsSupplyIntf.(uint64), nil
	}
...
func (s *state) syncGenesis(genesisBlk blocks.Block, genesis *genesis.State) error {
	...
	for _, vdrTx := range genesis.Validators {
		...
		s.AddTx(vdrTx, status.Committed, ids.Empty)
	}

	currentSupp := genesis.InitialSupply
	rewardsPoolSupp := uint64(0)
	if genesis.RewardsPoolSupply > totalRewards {
		rewardsPoolSupp = genesis.RewardsPoolSupply - totalRewards
	} else {
		currentSupp += totalRewards - genesis.RewardsPoolSupply
	}
	s.SetCurrentSupply(constants.PrimaryNetworkID, currentSupp)
	s.SetRewardsPoolSupply(constants.PrimaryNetworkID, rewardsPoolSupp)
	...
	for _, chain := range genesis.Chains {
		...
		if unsignedChain.NetworkID != s.ctx.NetworkID {
			return june.ErrWrongNetworkID
		}

		s.AddChain(chain)
		s.AddTx(chain, status.Committed, ids.Empty)
	}
...
func (s *state) loadCurrentValidators() error {
	...
	for validatorIt.Next() {
		...
		s.validatorUptimes.LoadUptime(staker.NodeID, staker.SupernetID, uptime)
	}
	...
	for supernetValidatorIt.Next() {
		...
		storedBytes := supernetValidatorIt.Value()
		...
		s.validatorUptimes.LoadUptime(staker.NodeID, staker.SupernetID, uptimeReward)
	}
...
func (s *state) initValidatorSets() error {
	...
	for supernetID := range s.cfg.WhitelistedSupernets {
...
func (s *state) write(updateValidators bool, height uint64) error {
	...
	// force update pools at genesis height
	if height == 0 {
	...
	errs.Add(
		...
		s.WriteUptimes(s.currentValidatorList, s.currentSupernetValidatorList), // Must be called after writeCurrentStakers
		s.writeTXs(),
		s.writeRewardUTXOs(),
		s.writeUTXOs(),
		s.writeSupernets(),
		...
		for nodeID, validatorDiff := range validatorDiffs {
			...
			if validatorDiff.validatorModified {
				...
				if validatorDiff.validatorDeleted {
					...
					s.validatorUptimes.DeleteUptime(nodeID, supernetID)
				} else {
					...
					s.validatorUptimes.LoadUptime(nodeID, supernetID, vdr)
					isNewValidator = true
				}
			}
			...
			// We only track the current validator set of whitelisted supernets.
			if supernetID != constants.PrimaryNetworkID && !s.cfg.WhitelistedSupernets.Contains(supernetID) {
				continue
			}
