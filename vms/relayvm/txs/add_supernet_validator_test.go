...
func TestAddSupernetValidatorTxSyntacticVerify(t *testing.T) {
	...
	addSupernetValidatorTx.SyntacticallyVerified = false
	addSupernetValidatorTx.Validator.Supernet = ids.Empty
	stx, err = NewSigned(addSupernetValidatorTx, Codec, signers)
	require.NoError(err)
	err = stx.SyntacticVerify(ctx)
	require.Error(err)
	addSupernetValidatorTx.Validator.Supernet = supernetID

	// Case: No weight
	addSupernetValidatorTx.SyntacticallyVerified = false
	addSupernetValidatorTx.Validator.Wght = 0
	stx, err = NewSigned(addSupernetValidatorTx, Codec, signers)
	require.NoError(err)
	err = stx.SyntacticVerify(ctx)
	require.Error(err)
	addSupernetValidatorTx.Validator.Wght = validatorWeight
	...
	*input = oldInput
	...
	addSupernetValidatorTx.SyntacticallyVerified = false
	addSupernetValidatorTx.Validator.Supernet = constants.PrimaryNetworkID
	stx, err = NewSigned(addSupernetValidatorTx, Codec, signers)
	require.NoError(err)
	err = stx.SyntacticVerify(ctx)
