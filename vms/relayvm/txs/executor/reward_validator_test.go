...
func TestRewardValidatorTxExecuteOnCommit(t *testing.T) {
	...
	// Get old balances
	oldBalance, err := june.GetBalance(env.state, stakeOwners)
	require.NoError(err)
	...
	onCommitBalance, err := june.GetBalance(env.state, stakeOwners)
	require.NoError(err)
...
func TestRewardValidatorTxExecuteOnAbort(t *testing.T) {
	...
	onAbortBalance, err := june.GetBalance(env.state, stakeOwners)
	require.NoError(err)
...
func TestRewardDelegatorTxExecuteOnCommit(t *testing.T) {
	...
	env.state.AddTx(vdrTx, status.Committed, ids.Empty)
	env.state.PutCurrentDelegator(delStaker)
	env.state.AddTx(delTx, status.Committed, ids.Empty)
	...
	oldVdrBalance, err := june.GetBalance(env.state, vdrDestSet)
	require.NoError(err)
	oldDelBalance, err := june.GetBalance(env.state, delDestSet)
	require.NoError(err)
	...
	commitVdrBalance, err := june.GetBalance(env.state, vdrDestSet)
	require.NoError(err)
	...
	commitDelBalance, err := june.GetBalance(env.state, delDestSet)
	require.NoError(err)
