...
func (e *ProposalTxExecutor) RewardValidatorTx(tx *txs.RewardValidatorTx) error {
	...
	// If the reward is aborted, then the rewards pool should be increased.
	rewardsPoolSupply, err := e.OnAbortState.GetRewardsPoolSupply(stakerToRemove.SupernetID)
	if err != nil {
		return err
	}
	newRewardsPoolSupply, err := math.Add64(rewardsPoolSupply, stakerToRemove.PotentialReward)
	if err != nil {
		return err
	}
	e.OnAbortState.SetRewardsPoolSupply(stakerToRemove.SupernetID, newRewardsPoolSupply)
