...
var (
	...
	MaxDelegationFee = uint32(120000)
)
...
func verifyAddValidatorTx(
	...
	case tx.DelegationShares > MaxDelegationFee:
	...
	outs := make([]*june.TransferableOutput, len(tx.Outs)+len(tx.StakeOuts))
	copy(outs, tx.Outs)
...
func verifyAddSupernetValidatorTx(
	...
	_, err := GetValidator(chainState, tx.Validator.Supernet, tx.Validator.NodeID)
	if err == nil {
		return fmt.Errorf(
	...
	baseTxCreds, err := verifyPoASupernetAuthorization(backend, chainState, sTx, tx.Validator.Supernet, tx.SupernetAuth)
	if err != nil {
		return err
	}
