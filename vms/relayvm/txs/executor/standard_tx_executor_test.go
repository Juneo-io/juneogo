...
func TestStandardTxExecutorAddDelegator(t *testing.T) {
	...
	addMinStakeValidator := func(target *environment) {
		...
		target.state.AddTx(tx, status.Committed, ids.Empty)
...
func TestStandardTxExecutorAddSupernetValidator(t *testing.T) {
...

	{
		...
		err = tx.Unsigned.Visit(&executor)
		if err == nil {
			t.Fatal("should have failed because validator stops validating primary network earlier than supernet")
		}
	}
	...
	env.state.AddTx(addDSTx, status.Committed, ids.Empty)
	dummyHeight := uint64(1)
	...
	env.state.PutCurrentValidator(staker)
	env.state.AddTx(supernetTx, status.Committed, ids.Empty)
	env.state.SetHeight(dummyHeight)
	if err := env.state.Commit(); err != nil {
		t.Fatal(err)
	}

	{
		...
		if err == nil {
			t.Fatal("should have failed verification because validator already validating the specified supernet")
		}
	}
...

	{
		...
		tx, err := env.txBuilder.NewAddSupernetValidatorTx(
			...
			ids.NodeID(nodeID), // node ID
			testSupernet1.ID(), // supernet ID
			[]*crypto.PrivateKeySECP256K1R{testSupernet1ControlKeys[0], testSupernet1ControlKeys[1], testSupernet1ControlKeys[2]},
			ids.ShortEmpty, // change addr
		)
		if err != nil {
			t.Fatal(err)
...

	{
		...
		tx, err := env.txBuilder.NewAddSupernetValidatorTx(
			...
			ids.NodeID(nodeID), // node ID
			testSupernet1.ID(), // supernet ID
			[]*crypto.PrivateKeySECP256K1R{testSupernet1ControlKeys[0], testSupernet1ControlKeys[2]},
			ids.ShortEmpty, // change addr
		)
		if err != nil {
			t.Fatal(err)
...

	{
		...
		tx, err := env.txBuilder.NewAddSupernetValidatorTx(
			...
			ids.NodeID(nodeID), // node ID
			testSupernet1.ID(), // supernet ID
			[]*crypto.PrivateKeySECP256K1R{testSupernet1ControlKeys[0], preFundedKeys[1]},
			ids.ShortEmpty, // change addr
		)
		if err != nil {
			t.Fatal(err)
...

	{
		...
		err = tx.Unsigned.Visit(&executor)
		if err == nil {
			t.Fatal("should have failed verification because validator already in pending validator set of the specified supernet")
		}
	}
}
	...
	tests := []test{
		...
		},
		{
			...
			newExecutor: func(ctrl *gomock.Controller) (*txs.RemoveSupernetValidatorTx, *StandardTxExecutor) {
				...
				env.fx.EXPECT().VerifyPermission(gomock.Any(), env.unsignedTx.SupernetAuth, env.tx.Creds[len(env.tx.Creds)-1], supernetOwner).Return(errors.New(""))
				e := &StandardTxExecutor{
					Backend: &Backend{
						Config: &config.Config{
							BanffTime: env.banffTime,
	...
	tests := []test{
		...
		},
		{
			...
			newExecutor: func(ctrl *gomock.Controller) (*txs.TransformSupernetTx, *StandardTxExecutor) {
				...
				env.state.EXPECT().SetCurrentSupply(env.unsignedTx.Supernet, 0)
				env.state.EXPECT().DeleteUTXO(gomock.Any()).Times(len(env.unsignedTx.Ins))
				env.state.EXPECT().AddUTXO(gomock.Any()).Times(len(env.unsignedTx.Outs))
				e := &StandardTxExecutor{
					Backend: &Backend{
