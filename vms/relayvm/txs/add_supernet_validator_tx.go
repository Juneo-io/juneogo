...
type AddSupernetValidatorTx struct {
	...
	// The validator
	Validator validator.SupernetValidator `serialize:"true" json:"validator"`
	// Auth that will be allowing this validator into the network
	SupernetAuth verify.Verifiable `serialize:"true" json:"supernetAuthorization"`
}

func (tx *AddSupernetValidatorTx) SupernetID() ids.ID {
	return tx.Validator.Supernet
}
...
func (tx *AddSupernetValidatorTx) StartTime() time.Time {
	return tx.Validator.StartTime()
}

func (tx *AddSupernetValidatorTx) EndTime() time.Time {
	return tx.Validator.EndTime()
}

func (tx *AddSupernetValidatorTx) Weight() uint64 {
...
func (tx *AddSupernetValidatorTx) SyntacticVerify(ctx *snow.Context) error {
	...
	case tx.SyntacticallyVerified: // already passed syntactic verification
		return nil
	case tx.Validator.Supernet == constants.PrimaryNetworkID:
