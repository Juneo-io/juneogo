...
type Client interface {
	...
	// GetRewardsPoolSupply returns the current rewards pool supply
	...
	// ExportJUNE issues an ExportTx transaction and returns the txID
	ExportJUNE(
		ctx context.Context,
		user api.UserPass,
		from []ids.ShortID,
	...
	// ImportJUNE issues an ImportTx transaction and returns the txID
	ImportJUNE(
		ctx context.Context,
		user api.UserPass,
		from []ids.ShortID,
	...
	// GetBlockchain returns the details of blochain with ID: [blockchainID] on the platform
	GetBlockchain(ctx context.Context, blockchainID string, options ...rpc.Option) (APIBlockchain, error)
...
func (c *client) GetFeesPoolValue(ctx context.Context, options ...rpc.Option) (uint64, error) {
	...
	err := c.requester.SendRequest(ctx, "getFeesPoolValue", struct{}{}, res, options...)
...
func (c *client) ExportJUNE(
	...
	res := &api.JSONTxID{}
	err := c.requester.SendRequest(ctx, "platform.exportJUNE", &ExportJUNEArgs{
		JSONSpendHeader: api.JSONSpendHeader{
			UserPass:       user,
			JSONFromAddrs:  api.JSONFromAddrs{From: ids.ShortIDsToStrings(from)},
			JSONChangeAddr: api.JSONChangeAddr{ChangeAddr: changeAddr.String()},
...
func (c *client) ImportJUNE(
	...
	res := &api.JSONTxID{}
	err := c.requester.SendRequest(ctx, "platform.importJUNE", &ImportJUNEArgs{
		JSONSpendHeader: api.JSONSpendHeader{
			UserPass:       user,
			JSONFromAddrs:  api.JSONFromAddrs{From: ids.ShortIDsToStrings(from)},
			JSONChangeAddr: api.JSONChangeAddr{ChangeAddr: changeAddr.String()},
...
func (c *client) GetBlockchain(ctx context.Context, blockchainID string, options ...rpc.Option) (APIBlockchain, error) {
	res := &GetBlockchainResponse{}
	err := c.requester.SendRequest(ctx, "getBlockchain", &GetBlockchainArgs{
		BlockchainID: blockchainID,
	}, res, options...)
	return res.Blockchain, err
}
