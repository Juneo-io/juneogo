...
import (
	...
	"github.com/Juneo-io/juneogo/vms/relayvm/txs/txheap"
...
// [ChainAssetID] is the main asset of the chain used to pay for the fees.
type Chain struct {
...
func (*StaticService) BuildGenesis(_ *http.Request, args *BuildGenesisArgs, reply *BuildGenesisReply) error {
	...
	for i, apiUTXO := range args.UTXOs {
		...
		utxo := june.UTXO{
			...
			Asset: june.Asset{ID: args.JuneAssetID},
		...
		if apiUTXO.Locktime > args.Time {
			utxo.Out = &stakeable.LockOut{
				...
				TransferableOut: utxo.Out.(june.TransferableOut),
			}
		}
	...
	for _, vdr := range args.Validators {
		weight := uint64(0)
		stake := make([]*june.TransferableOutput, len(vdr.Staked))
