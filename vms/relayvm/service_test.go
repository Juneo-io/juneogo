...
import (
	...
	pchainapi "github.com/Juneo-io/juneogo/vms/relayvm/api"
...
func TestGetTxStatus(t *testing.T) {
	...
	peerSharedMemory := m.NewSharedMemory(assetChainID)
	...
	tx, err := service.vm.txBuilder.NewImportTx(assetChainID, ids.ShortEmpty, []*crypto.PrivateKeySECP256K1R{recipientKey}, ids.ShortEmpty)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetStake(t *testing.T) {
	...
	for i, validator := range genesis.Validators {
		...
		var output june.TransferableOutput
	...
	service.vm.state.AddTx(tx, status.Committed, ids.Empty)
	...
	outputs := make([]june.TransferableOutput, 2)
	for i := range outputs {
	...
	// Unmarshal
	outputs = make([]june.TransferableOutput, 3)
	for i := range outputs {
		outputBytes, err := formatting.Decode(args.Encoding, response.Outputs[i])
		require.NoError(err)
		_, err = txs.Codec.Unmarshal(outputBytes, &outputs[i])
